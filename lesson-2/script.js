...
/* ゲッターとセッターを設定
  ...
  //get・ゲッター・・・プロパティの値を取得するためのメソッド  ・値を変更する必用がなければget だけで十分
  //set・セッター・・・プロパティの値を設定するためのメソッド  ・set はあくまで後から値を変更したいときに使えばいい

  {
    class User {
      //フィールド
      name;
      _score;  //ゲッターやセッターを使う場合、内部的に値を保持するプロパティは別の名前にするのが一般的で、慣習的に先頭に _ を付ける
